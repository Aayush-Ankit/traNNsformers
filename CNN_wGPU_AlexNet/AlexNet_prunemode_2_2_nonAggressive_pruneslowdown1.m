...
for e = continuationEpoch+1:opts.numepochs
    ...
    optsTrain = trainingOptions('sgdm', ...
        ...
        'OutputFcn', @(info)getMiniBatchLoss2(info), ...
    ...
    loss.train.e(end+1) = gather(averageEpochLoss2);
    ...
    if (opts.learnRateDropActive == 1)
        if (((loss.train.e(end-1)-loss.train.e(end)) > 0) && (loss.train.e(end-1)-loss.train.e(end)) < opts.lossTolerance)
    ...
    if (opts.tr_err_prev > loss.train.e(end) && opts.pruneANDclusterStart == 0)
        ...
        opts.clusterstartepoch = ceil(0.2 * (opts.pruneANDclusterStart)) + opts.pruneANDclusterStart; % epoch when clustering map is created (need to start from somewhat pruned map)
