function [ num_mpe, avg_util, num_mpe_unclustered ] = get_hardware_params_cnn(cnn, opts, prune, prunemode)
...
% prunemode - which mode was used (no pruning, pruning, clustered pruning)
    ...
    num_mpe = zeros(opts.numberFCLayers,1);
    num_mpe_unclustered = zeros(opts.numberFCLayers,1);
    avg_util = zeros(opts.numberFCLayers,1);
    ...
    switch prunemode
        ...
        case 0
            ...
            for i = 1:opts.numberFCLayers
                num_mpe(i) = ceil(size(cnn(opts.fcIndices(i)).Weights, 1)/xbar_size) * ceil(size(cnn(opts.fcIndices(i)).Weights, 2)/xbar_size);
                avg_util(i) = 1;
            end
        ...
        case 2
            ...
            for i = 1:opts.numberFCLayers
                ...
                unclustered_map = (prune.pmap{i} == 1) & (prune.cmap{i} == 0);
