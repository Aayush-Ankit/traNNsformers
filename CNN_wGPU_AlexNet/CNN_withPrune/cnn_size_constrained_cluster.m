function [ clusters ] = cnn_size_constrained_cluster(conn_matrix, crossbar_size, base_quality, debug)
...
%sizes are less than a certain value (crossbar size (rows+column))
    ...
    % setup the problem
    m = 2; % m*crossbar_size is the constraint
    constraint = m*crossbar_size;
    
    % run sc on the conn_matrix
    [clusters, ~] = cnn_cluster(conn_matrix, base_quality);
    ...
    while (constraint_met ~= 0) 
        ...
        clusters_p = clusters;
        clusters = {};
        clusters = cnn_subcluster(clusters_p, constraint, base_quality);
    ...
    if_hist = 0;
    cnn_analyse_cluster (clusters, conn_matrix, if_hist);
