...
% TRANNSFORMER
    ...
    cnn.cluster_prune_start = 0.7* opts.numepochs;
...
fid = fopen([sprintf('CNNoutputs/softwarePerspective_AccuracyVsTrainingEffortAnalysis/%s/trace_prunemode%d_noPruneSlowdown_earlyClusterPrune_numlayers%d_with%dFClayers_numfilters%dand%d_learningRate%2.2f_xbarutilmin%0.2f',...
...
cnn = cnntrain_addPrune_2(cnn, train_x, train_y, opts, test_x, test_y);
...
if_hist = 1;
    ...
    saveas(fig, sprintf('CNNoutputs/softwarePerspective_AccuracyVsTrainingEffortAnalysis/%s/hist_prunemode%d_noPruneSlowdown_earlyClusterPrune_numlayers%d_with%dFClayers_xbarutilmin%0.2f.png',...
        data_name, prunemode, (cnn.n-1), cnn.FCcounter, cnn.cluster_base_quality_min))
end

%% Saving Output Data

save ([sprintf('CNNoutputs/softwarePerspective_AccuracyVsTrainingEffortAnalysis/%s/trace_prunemode%d_noPruneSlowdown_earlyClusterPrune_numlayers%d_with%dFClayers_numfilters%dand%d_learningRate%2.2f_xbarutilmin%0.2f',... 
