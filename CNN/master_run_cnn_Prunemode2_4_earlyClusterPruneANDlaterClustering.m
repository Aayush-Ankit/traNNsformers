...
% TRANNSFORMER
    cnn.clusterstartepoch = 0.3* opts.numepochs; % epoch when clustering map is created (need to start from somewhat pruned map)
...
fid = fopen([sprintf('CNNoutputs/%s/trace_prunemode%d_noPruneSlowdown_earlyCluterPruneANDlaterClustering_numlayers%d_with%dFClayers_numfilters%dand%d_learningRate%2.2f_xbarutilmin%0.2f',...
...
if_hist = 1;
    ...
    saveas(fig, sprintf('CNNoutputs/%s/hist_prunemode%d_noPruneSlowdown_earlyCluterPruneANDlaterClustering_numlayers%d_with%dFClayers_xbarutilmin%0.2f.png',...
        data_name, prunemode, (cnn.n-1), cnn.FCcounter, cnn.cluster_base_quality_min))
end

%% Saving Output Data

save ([sprintf('CNNoutputs/%s/trace_prunemode%d_noPruneSlowdown_earlyCluterPruneANDlaterClustering_numlayers%d_with%dFClayers_numfilters%dand%d_learningRate%2.2f_xbarutilmin%0.2f',... 
