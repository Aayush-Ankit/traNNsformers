function nn = run_fcn_general(data_name, dataset_pathid, net, epochs, prune_slowdown, prunemode, testAccuracyEveryEpoch)
    ...
    fid = fopen(sprintf('output/softwarePerspective_AccuracyVsTrainingEffortAnalysis/%s/trace_prunemode%d_numlayers%d_xbarutilmin%0.2f.txt', data_name, prunemode, nn.n-1, nn.cluster_base_quality_min), 'w');
    ...
    if (nn.prunemode == 2)
        ...
        saveas(fig, sprintf('output/softwarePerspective_AccuracyVsTrainingEffortAnalysis/%s/hist_prunemode%d_numlayers%d_xbarutilmin%0.2f.png', data_name, prunemode, nn.n-1, nn.cluster_base_quality_min))
    end
    save (sprintf('output/softwarePerspective_AccuracyVsTrainingEffortAnalysis/%s/nn_prunemode%d_numlayers%d_xbarutilmin%0.2f.mat', data_name, prunemode, nn.n-1, nn.cluster_base_quality_min), 'nn', 'opts', 'accuracyVStrainingEffort');
