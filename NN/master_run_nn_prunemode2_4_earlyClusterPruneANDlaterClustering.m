...
dirspec = 'output/resultset3_sensitivytToParamVariation/%s';
epochs_mnist = 60;
...
opts.testAccuracyEveryEpoch = testAccuracyEveryEpoch;
...
opts.batchsize = 400;
nn.clusterstartepoch = 0.3* opts.numepochs; % epoch when clustering map is created (need to start from somewhat pruned map)
...
nn.cluster_prune_start = 0.7* opts.numepochs;
nn.scale_clusterpruneRate = [0.01 0.01 0.01 0.01];

%% Training with Iterative Clustering + Pruning
fid = fopen(sprintf('output/resultset3_sensitivytToParamVariation/%s/trace_prunemode%d_4_noPruneSlowdown_laterClusterANDearlyClusterPrune_numlayers%d_xbarutilmin%0.2f.txt', data_name, prunemode, nn.n-1, nn.cluster_base_quality_min), 'w');
...
if opts.testAccuracyEveryEpoch == 1
    fprintf(fid, 'Test Accuracy FLAG is ON. Verifying and Displaying accuracy every epoch.\n');
end

% Train - takes about 15 seconds per epoch on my machine
[nn, ~, accuracyVStrainingEffort] = nntrain(nn, train_x, train_y, opts, test_x, test_y);
...
if (nn.prunemode == 2)
    ...
    saveas(fig, sprintf('output/resultset3_sensitivytToParamVariation/%s/hist_Prunemode%d_4_noPruneSlowdown_laterClusterANDearlyClusterPrune_numlayers%d_xbarutilmin%0.2f.png', data_name, prunemode, nn.n-1, nn.cluster_base_quality_min))
end
save (sprintf('output/resultset3_sensitivytToParamVariation/%s/nn_prunemode%d_4_noPruneSlowdown_laterClusterANDearlyClusterPrune_numlayers%d_xbarutilmin%0.2f.mat', data_name, prunemode, nn.n-1, nn.cluster_base_quality_min), 'nn', 'opts', 'accuracyVStrainingEffort');
