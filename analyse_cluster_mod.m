function qual_vec_t = analyse_cluster_mod(clusters, conn_matrix, if_hist)
...
%     frac_unclustered_synapses = 1 - sum(sum(cluster_map .* conn_matrix))/ sum(sum(conn_matrix)); % conn_matrix the logical or of pmap and cmap
    ...
    [h,k] = size(unclus_map);
    frac_unclustered_synapses = sum(sum(unclus_map))/ (sum(sum(conn_matrix)));
    
    
    %fprintf(fid, 'Fraction of synapses unclustered = %0.4f\n', frac_unclustered_synapses);
    fprintf('Fraction of synapses unclustered = %0.4f\n', frac_unclustered_synapses);
    if (if_hist == 1)
        % only plot the hist of non-zero qual_vec (zero Q represents an invalidated cluster)
        qual_vec_t = qual_vec(1:idx);
